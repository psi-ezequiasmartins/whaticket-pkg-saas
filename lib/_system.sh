...
#######################################
# Verifica se serviços essenciais estão ativos
# Arguments:
#   Lista de serviços (snapd, nginx, docker)
#######################################
system_check_services() {
  local services=("snapd" "nginx" "docker")
  for svc in "${services[@]}"; do
    systemctl is-active --quiet $svc
    if [ $? -eq 0 ]; then
      echo "✅ Serviço $svc está ativo."
    else
      echo "❌ Serviço $svc NÃO está ativo! Tente iniciar com: sudo systemctl start $svc"
    fi
  done
}
...
system_certbot_install() {
  echo "🔄 Instalando/Atualizando snapd e certbot..."
  ...
  # Garante que o serviço snapd está rodando
  ...
  # Cria link simbólico para certbot, se não existir
...
system_create_user() {
  print_banner
  printf "${WHITE} 💻 Agora, vamos criar o usuário para whaticketsaas...${GRAY_LIGHT}"
  printf "\n\n"
  ...
  if ! id "deploy" &>/dev/null; then
    # Cria usuário com home, shell bash e grupo sudo
...
# Uncomment the following function if you want to use it
# Note: This function is commented out to avoid conflicts with the above function.
# #######################################
# # installs certbot
# # Arguments:
# #   None
